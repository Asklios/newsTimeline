...
def create_missing_tables():
    ...
    conn.run("CREATE TABLE IF NOT EXISTS news_complete("
             ...
             "source_url TEXT,"
             "content TEXT)")
    conn.run("CREATE TABLE IF NOT EXISTS news_clean_uppercase("
             "id SERIAL PRIMARY KEY,"
...
def get_news_sources():
    """
    Selects all news sources from news_sources table.
    [source, source_url]

    :return: result tuple, can be empty if no stored news_sources are available
    """
    try:
        params = config()
        conn: Connection = pg8000.dbapi.Connection(**params)
...
def save_complete_news(time: datetime, source: str, source_url: str, content: str):
    """
    Inserts data into the news_complete table.
    [time, source, source_url, content]
    """
